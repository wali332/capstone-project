...
export default function LiveDemo() {
  ...
  // Terminal animation while API call runs
  useEffect(() => {
    if (state === 'ANALYZING') {
      let currentLine = 0;
      setLogLines([]);
      ...
      const intervalId = setInterval(() => {
        ...
        // Don't auto-advance — wait for the real API response
      }, 350);
  ...
  const startAnalysis = async (file) => {
    ...
    // File size displayed immediately from the File object
    ...
    try {
      ...
      if (msg.includes('Failed to fetch') || msg.includes('NetworkError')) {
        msg = 'Cannot reach backend. Is the FastAPI server running? (python run.py)';
      } else if (msg.includes('503')) {
        msg = 'Model not loaded yet. Run: python scripts/train.py';
      }
  ...
  return (
    <section id="try-it" className="w-full py-32 bg-brand-surface relative overflow-hidden">
      <div className="container mx-auto px-8 max-w-[1000px] flex flex-col items-center">
        ...
        {/* Error banner */}
        {error && (
          <div className="w-full max-w-5xl mb-4 px-5 py-3 bg-red-900/30 border border-red-500/40 rounded-[8px] font-mono text-[12px] text-red-400">
            {error}
          </div>
        )}
              ...
              {(state === 'IDLE' || state === 'ANALYZING') && (
                ...
                >
                  ...
                  >
                    {state === 'IDLE' ? (
                      ...
                      <div className="flex flex-col items-center gap-4">
                        ...
                        <span className="font-mono text-[11px] text-brand-violet">Analyzing...</span>
                      </div>
                    )}
                  </div>
                  ...
                  {state === 'ANALYZING' && (
                    <button
                      onClick={reset}
                      className="mt-6 text-[13px] text-gray-500 hover:text-white outline-none transition-colors"
                    >
                      cancel
                    </button>
                  )}
